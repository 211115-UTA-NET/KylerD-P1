    ...
    public class EmployeeTransactionByStore : IViewTransaction
    {
        ...
        public static void TransactionHistory(int myEntry)
        {
            ...
            while (exit == false)
            {
                ...
                try
                {
                    transList = SpiceItUpDataStorage.SqlRepository.StoreTransactionList(myEntry);
                }
                catch (Exception)
                {
                ...
                while (true) //The employee can now view a store's transaction in more detail, if they wish
                {
                    ...
                    {
                        SpiceItUpDataStorage.SqlRepository.DetailedTransaction(userEntry); //Pull results of transaction from database and print for employee
                        break;
                    }
