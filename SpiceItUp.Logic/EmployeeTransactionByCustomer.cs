    ...
    public class EmployeeTransactionByCustomer : IViewTransaction
    {
        ...
        public static void SelectACustomer()
        {
            ...
            while (exit == false)
            {
                ...
                try
                {
                    customerIDList = SpiceItUpDataStorage.SqlRepository.CustomerTransactionList();
                }
                catch (Exception)
                {
        ...
        public static void TransactionHistory(int myEntry)
        {
            ...
            while (goBack == false)
            {
                ...
                try
                {
                    ...
                    transList = SpiceItUpDataStorage.SqlRepository.CustomerTransactionHistory(customerID);
                }
                catch(Exception)
                {
                ...
                while (true) //The employee is prompted to choose a transaction to view in more detail
                {
                    ...
                    {
                        SpiceItUpDataStorage.SqlRepository.DetailedTransaction(userEntry2); //View the transaction in more detail
                        break;
                    }
